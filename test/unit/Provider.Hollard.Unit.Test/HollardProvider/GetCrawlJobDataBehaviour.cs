...
{
    public class GetCrawlJobDataBehaviour : HollardProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(HollardConstants.KeyName.ApiKey, nameof(HollardCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task HollardCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Hollard.Core.HollardCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
